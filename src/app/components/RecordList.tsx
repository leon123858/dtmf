...
import { prepareRecordRows } from '@/app/lib/recordList';
...
export function RecordList({ onEdit }: { onEdit: (record: Record) => void }) {
	...
	const rows = useMemo(() => prepareRecordRows(records ?? []), [records]);
	const rowVirtualizer = useVirtualizer({
		count: rows.length,
		getItemKey: index => rows[index].record.id,
		getScrollElement: () => parentRef.current,
		estimateSize: index => CARD_HEIGHT + (rows[index].separator ? SEPARATOR_HEIGHT : 0),
		overscan: 5,
	});

	useEffect(() => { rowVirtualizer.measure(); }, [rows, rowVirtualizer]);
	...
	return <div className='flex min-h-0 flex-1 flex-col gap-3'>
		...
		<div ref={parentRef} role='region' aria-label='帳目列表' tabIndex={-1} className='min-h-0 flex-1 overflow-y-auto overscroll-contain'>
			<div role='list' aria-label='帳目' className='relative w-full' style={{ height: rowVirtualizer.getTotalSize() }}>
				{rowVirtualizer.getVirtualItems().map(item => {
					...
					return <div key={item.key} role='listitem' aria-posinset={item.index + 1} aria-setsize={rows.length} data-index={item.index} ref={rowVirtualizer.measureElement}
						className='absolute top-0 left-0 w-full pb-3' style={{ transform: `translateY(${item.start}px)` }}>
